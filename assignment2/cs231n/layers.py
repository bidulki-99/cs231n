...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        var = 1. / N * np.sum(dev_from_mean_sq, axis = 0)
...
def batchnorm_backward(dout, cache):
    ...
    beta, gamma = cache['beta'], cache['gamma']
    x_norm, var = cache['x_norm'], cache['var']
    eps, dev_from_mean = cache['eps'], cache['dev_from_mean']
    stddev, inverted_stddev = cache['stddev'], cache['inverted_stddev']
    x, mean, axis = cache['x'], cache['mean'], cache['axis']
...
def spatial_groupnorm_backward(dout, cache):
    ...
    size, G, scaled_x = cache['size'], cache['G'], cache['scaled_x']
